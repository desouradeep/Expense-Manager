...
class app:
    ...
    def __init__(self):
        ...
        self.window.set_default_size(1230,658)
        ...
        if option==1:
          ...
          for i in model:
            for j in i:
              counter+=1
              if counter%7==0:
                self.total_list.append(j)
              #print j, type(j),
            #print '\n' 
          #print range(len(self.total_list))
          #print self.total_list
          if max(self.total_list)==0:
            M=1
          else:
            M=max(self.total_list)+0.1*max(self.total_list)
          self.a.set_ylim(0,M)
          self.a.set_xlim(1,len(self.total_list)-1)
          days=[]
          ...
          #print self.total_list, len(self.total_list)        
          #self.total_list.append(100)
          while len(self.line1)!=0:
            l=self.line1.pop(0)
            l.remove()
          self.total_list.append(0)
          #self.a.set_antialiased(False)
          self.line1=self.a.fill(self.total_list,'blue',alpha=0.6)
          ...
          if max(self.total_list)==0:
            ...
            M=max(self.total_list)+0.1*max(self.total_list)
          self.b.set_ylim(0,M)
          self.b.set_xlim(-0.5,4.5)
          self.b.set_xticks([0,1,2,3,4])
          ...
          self.total_list.append(0)
          #self.a.set_antialiased(False)
          self.line1b=self.b.bar(range(len(self.total_list)),self.total_list,align='center',facecolor='yellow',alpha=0.5)
          self.canvas.draw()
